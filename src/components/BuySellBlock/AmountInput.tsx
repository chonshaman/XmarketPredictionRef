...
const QuickAmountButton = React.memo<{ label: string; onClick: () => void }>(({ label, onClick }) => {
  ...
  return (
    <motion.div 
      ...
      className="box-border content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[9999px] shrink-0 cursor-pointer transition-colors"
      style={{
...
export const AmountInput = React.memo<AmountInputProps>(({
  ...
  return (
    <div 
      className="box-border content-stretch flex flex-col gap-[8px] items-start pb-[12px] pt-[4px] relative rounded-[12px] shrink-0 w-full border cursor-text transition-all duration-200 ease-out"
      onClick={handleContainerClick}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      style={{
        paddingLeft: currentState === "focused" ? "16px" : currentState === "hover" ? "12px" : currentState === "error" ? "14px" : "4px",
        paddingRight: currentState === "focused" ? "16px" : currentState === "hover" ? "12px" : currentState === "error" ? "14px" : "4px",
