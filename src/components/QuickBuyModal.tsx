...
export function QuickBuyModal({ isOpen, onClose, market, prediction, cardPosition, onSuccess }: QuickBuyModalProps) {
  ...
  const chance = prediction === 'yes' ? market.yesPercentage || 59 : market.noPercentage || 41;
  ...
  // Calculate modal dimensions and positioning
  const modalHeight = 430; // Approximate height of the modal
  const modalWidth = isMobile ? (typeof window !== 'undefined' ? window.innerWidth * 0.96 : 300) : cardPosition.width * 1.08;
  
  // Center the modal vertically on the card
  let targetTop = cardPosition.top + (cardPosition.height / 2) - (modalHeight / 2);
  
  // Ensure modal doesn't go off-screen
  const viewportHeight = typeof window !== 'undefined' ? window.innerHeight : 800;
  const minTop = 20; // 20px from top
  const maxTop = viewportHeight - modalHeight - 20; // 20px from bottom
  
  // Clamp the top position
  targetTop = Math.max(minTop, Math.min(targetTop, maxTop));
  
  // Calculate left position
  const targetLeft = isMobile ? (typeof window !== 'undefined' ? window.innerWidth * 0.02 : 10) : cardPosition.left - 12;
    ...
    <AnimatePresence mode="wait">
      {isOpen && (
        <>
          ...
          {/* Modal positioned at card location */}
          <motion.div
            key="quick-buy-modal"
            initial={{ 
              opacity: 0,
              top: cardPosition.top,
              left: isMobile ? '2%' : cardPosition.left,
